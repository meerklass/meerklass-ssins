#!/bin/bash
#SBATCH --job-name=ssinsR1
#SBATCH --array=0  # SLURM job array index value, running job 0 to job 10 here
...
#SBATCH --output=logs/ssinsl0-%A-%a.out#### Note ####
...
export OMP_NUM_THREADS=${SLURM_CPUS_PER_TASK}# Activate conda and the correct Python environment
source ~/miniforge3/bin/activate
conda activate katcali
echo "Using Python: $(which python)"# Pass variable SLURM_ARRAY_TASK_ID, which is the job array index value
echo "Executing a command: multi-block-Analysis.py ${SLURM_ARRAY_TASK_ID}"
python executable_ssins.py ${SLURM_ARRAY_TASK_ID}
