...
def visData(fname, ant, pol, verbos=False):
    """This function  will access the data block and return visibility data, flags, 
    noise diodes vector.
    ...
    dp_tt, dp_ss, dp_f, dp_w, dp_t, dp_s, dp_slew, dp_stop = kl.cal_dp_label(
        data, flags, ant, pol, ch_ref, ang_deg
    )
    ...
    nd_ratio, nd_0, nd_1x = kd.cal_nd_ratio(
        timestamps, nd_on_time, nd_on_edge, data.dump_period
    )

    nd_t0, nd_t1x, nd_s0, nd_s1x, nd_t0_ca, nd_t0_cb, nd_t1x_ca, nd_t1x_cb = (
        kl.cal_label_intersec(dp_tt, dp_ss, nd_0, nd_1x)
    )
...
def MaskedArrayVisibilityFlags(vis, pipeline_flags, nd_s0, pointsource_flags=None):
    ...
    nd_flags[nd_s0, :] = (
        False  # Set the data with noise diodes off to False so that this data is not flagged as bad data.  This is the  scan only data.
    )
...
def plot(
    ...
    if (
        ax is None
    ):  # Create a new figure and axes if not being, passed in as a parameter
        fig, ax = plt.subplots(1, 1, figsize=figsize)
...
def mask_to_flags(nd_s0, zscore_mask, ants, nd_flags=None, pipeline_flags=None):
    ...
    flags_new = np.zeros(
        [shape[0] + 3] + shape[1:], dtype=bool
    )  # (t, f) ----> (time, frequency)  # expanded the dims
